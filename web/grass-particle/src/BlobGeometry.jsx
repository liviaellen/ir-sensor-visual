...
export const BlobGeometry = forwardRef((props, ref) => {
  const geomery = useMemo(() => {
    const geom = new THREE.IcosahedronGeometry(1.0, 16);
    ...
    for (let i = 0; i < positionAttribute.count; i++) {
      ...
      // Subtle surface noise — just enough for terrain texture, not blobby
      const v = vertex.clone().multiplyScalar(1.2);
      const n = Perlin.simplex3(...v.toArray());
      vertex.add(normal.clone().multiplyScalar(n * 0.055));

      // Slight polar flattening — Earth is ~0.3% wider at equator, exaggerate a little
      vertex.y *= 0.97;
  ...
  return (
    <mesh ref={ref} geometry={geomery}>
      {/* <ComputedAttribute
        attach="geometry-attributes-density"
        name="density"
